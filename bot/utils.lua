...
local function sudoset(extra, user_id)
    ...
    if _config.sudo_users[uid] then
      ...
      save_config()
      reply_msg(extra.msg.id, extra.usr..' has been promoted', ok_cb, true)
    end
  end

  local function sudodem(extra, user_id)
    local uid = tonumber(user_id)
    if not _config.sudo_users[uid] then
      reply_msg(extra.msg.id, extra.usr..' is not a sudouser.', ok_cb, true)
    elseif uid == extra.msg.from.peer_id then
      ...
      save_config()
      reply_msg(extra.msg.id, extra.usr..' has been Demoted', ok_cb, true)
    end
  end
